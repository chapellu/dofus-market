...
from grpc_market.grpc.grpc_market_p2p import (MetierDestroyRequest,
                                              MetierListRequest,
                                              MetierListResponse,
                                              MetierRequest, MetierResponse,
                                              MetierRetrieveRequest)
from grpc_market.grpc.grpc_market_pb2_grpc import (
    MetierControllerStub, add_MetierControllerServicer_to_server)
...
@pytest.mark.asyncio
@pytest.mark.django_db(transaction=True)
async def test__retrieve_metier__ok(grpc_client,
...
@pytest.mark.asyncio
@pytest.mark.django_db(transaction=True)
async def test__create_metier__ok(grpc_client,
                                  populate_database_with_test_data):
    # Given
    await populate_database_with_test_data
...
@pytest.mark.asyncio
@pytest.mark.django_db(transaction=True)
async def test__create_metier__already_exist(grpc_client,
                                             populate_database_with_test_data):
    # Given
    await populate_database_with_test_data
    ...
    assert json.loads(details) == {
        "name": [{
            "message": "metier with this name already exists.",
            "code": "unique"
        }]
    }
...
@pytest.mark.asyncio
@pytest.mark.django_db(transaction=True)
async def test__destroy_metier__ok(grpc_client,
                                   populate_database_with_test_data):
    # Given
    await populate_database_with_test_data
