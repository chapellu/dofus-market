...
from grpc_market.grpc.grpc_market_p2p import (
    IngredientForCraftDestroyRequest, IngredientForCraftListRequest,
    IngredientForCraftListResponse, IngredientForCraftRequest,
    IngredientForCraftResponse, IngredientForCraftRetrieveRequest)
...
async def test__ingredient_for_craft_to_string(
...
async def test__list_ingredient_for_craft__empty_list__ok(grpc_client):
    ...
    response: pb2.IngredientForCraftListResponse = await grpc_client.List(
        request)
...
async def test__list_ingredient_for_craft__list_with_2_items__ok(
    ...
    for i in range(len(ingredient_for_crafts)):
        assert response.results[i].ingredient == ingredient_for_crafts[
            i].ingredient.name
        assert response.results[i].quantity == ingredient_for_crafts[
            i].quantity
...
async def test__retrieve_ingredient_for_craft__ok(
    ...
    grpc_response: pb2.IngredientForCraftResponse = await grpc_client.Retrieve(
        request)
...
async def test__create_ingredient_for_craft__ok(
    ...
    # When
    request = IngredientForCraftRequest(ingredient=ingredient_name,
                                        quantity=quantity)
...
async def test__create_ingredient_for_craft__already_exist(
    ...
    assert json.loads(details) == {
        "non_field_errors": [{
            "message":
            "The fields ingredient, quantity must make a unique set.",
            "code": "unique"
        }]
    }
...
async def test__update_ingredient_for_craft__ok(
    ...
    # When
    request = IngredientForCraftRequest(id=craft_ressource1.pk,
    ...
    ingredient_for_craft = await IngredientForCraft.objects.aget(
        id=craft_ressource1.pk)
