...
from grpc_market.grpc.grpc_market_pb2 import EquipementResponse, EquipementListResponse, EquipementDetailsResponse
from asgiref.sync import sync_to_async


class EquipementProtoSerializer(proto_serializers.ModelProtoSerializer):
    cout_fabrication = serializers.FloatField(
        source="equipement_fabrication_cost")
...
class EquipementDetailsProtoSerializer(EquipementProtoSerializer):
    ...
    class Meta:
        model = OrderedByRentability
        fields = ("name", "level", "cout_fabrication", "gain_estime",
                  "rentabilite", "nb_objet", "metier", "effects",
                  "ingredients", "brisage")
        proto_class = EquipementDetailsResponse

    def serialize_effects(self, ordered_by_rentability: OrderedByRentability):
        return CaracteristiqueProtoSerializer(
            ordered_by_rentability.name._effects, many=True).data

    def serialize_ingredients(self,
                              ordered_by_rentability: OrderedByRentability):
        return IngredientforCraftProtoSerializer(
            ordered_by_rentability.name._ingredients, many=True).data

    def serialize_brisage(self, ordered_by_rentability: OrderedByRentability):
...
class IngredientforCraftProtoSerializer(proto_serializers.ProtoSerializer):
    ...
    price = serializers.SerializerMethodField(
        'serialize_ingredient_price_for_craft')
    ingredients = serializers.SerializerMethodField('serialize_ingredients')
    ...
    def serialize_ingredients(self, ingredient_for_craft: IngredientForCraft):
        try:
            recette = Recette.objects.get(
                pk=ingredient_for_craft.ingredient.pk)
        except Recette.DoesNotExist:
            return []
        return IngredientforCraftProtoSerializer(recette.ingredients,
                                                 many=True).data
    ...
    def serialize_cout_fabrication(self,
                                   ingredient_for_craft: IngredientForCraft):
        try:
            recette = Recette.objects.get(
                pk=ingredient_for_craft.ingredient.pk)
        except Recette.DoesNotExist:
    ...
    def serialize_rentabilite(self, ingredient_for_craft: IngredientForCraft):
        cout_fabrication = self.serialize_cout_fabrication(
            ingredient_for_craft)
        if cout_fabrication == 0:
            return 0.0
        else:
            return (ingredient_for_craft.ingredient.price - cout_fabrication
                    ) / ingredient_for_craft.ingredient.price * 100
